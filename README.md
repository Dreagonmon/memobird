# 咕咕机非官方 WEB API 调用
memobird官方的SDK必须从手机连接设备，无法从电脑直接调用，要打印自定义内容很麻烦。
同时部分型号不支持WIFI，无法通过官方的 WIFI API 接口进行调用。

本项目利用了官方网页版的API接口，使用用户信息进行登录，无需申请开发者，
可以直接将打印内容发送到云端，手机拉取到打印内容之后，通过蓝牙调用本地设备进行打印，
所以理论上只要支持手机连接的咕咕机都可以使用。
